...
tmpfilename='/tmp/ebpf-sdk-cli-input.json'
...
def askUserForValues(key, value):
    ...
    content = f.read()
    userProvided = json.loads(content)
    log.info('After user input - ')
    log.info(json.dumps(userProvided, indent=1))
    f.close()
...
def doCRUD(op, maps):
    ...
    raw_key = btf.generateRawValue(flatten_key)
    raw_value = btf.generateRawValue(flatten_value)
